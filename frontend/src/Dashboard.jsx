        ...
        <Accordion>
          {lists.map((list) => {
            return JSON.parse(list['shows']).map((show) => {
              const episodesForShow = episodes.filter((episode) => episode.show_id == show['id']);
              const watchedEpisodesForShow = watched.filter(
                (episode) => episode.showId == episode.show_id
              );
